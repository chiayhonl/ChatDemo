    ...
    public void run() {
        try {
            ...
            while(true) {
                ...
                // 提示框选择结果，0为确定，1位取消
                if(result == 0){
//					out.println("***系统提示:【" + userName + "】选择了接收文件！");
//					out.flush();
                    File userFile = new File("C:\\Users\\Administrator\\Desktop\\接收文件\\" + userName);
                    if(!userFile.exists()) {  // 新建当前用户的文件夹
                        userFile.mkdir();
                    }
                    File file = new File("C:\\Users\\Administrator\\Desktop\\接收文件\\" + userName + "\\"+ textName);
...
//						out.flush();
                    ...
                    out.println("***系统提示：【" + userName + "】接收了文件！***");
                    out.flush();
                    // 提示文件存放地址
    ...
    public static void outFileToServer(String path) {
        try {
            ...
            out.println("***系统提示：【" + userName + "】已成功发送文件！***");
            out.flush();
        } catch (Exception e) {}
    }
}
